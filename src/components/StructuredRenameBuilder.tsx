import { ArrowDown, ArrowLeft, ArrowRight, Copy, Info, Play, Plus, Trash2 } from 'lucide-react'
...
function isEditable(column: RenameColumn): boolean {
  return column.kind === 'literal' || column.kind === 'manual'
}

export function StructuredRenameBuilder({ files, busy, onRun, t }: StructuredRenameBuilderProps) {
  const [columns, setColumns] = useState<RenameColumn[]>([])
  ...
  const config = useMemo<StructuredRenameConfig>(() => ({ columns, sortMode, resolveCollisions }), [columns, resolveCollisions, sortMode])
  ...
  const importOriginalNames = (column: RenameColumn) => {
    setOverrides((current) => Object.fromEntries(previews.map((preview) => [preview.fileId, { ...current[preview.fileId], [column.id]: splitFileName(preview.before).stem }])))
  }
  const addColumn = () => {
  ...
  const changeColumnKind = (column: RenameColumn, kind: StructuredColumnKind) => {
    ...
    setColumns((current) => current.map((item) => item.id === column.id ? { ...replacement, id: item.id } : item))
  ...
  return <section className="structured-workspace">
    ...
    <div className="structured-toolbar">
      ...
      <div className="structured-global-options"><label className="structured-toolbar-select"><span>{t('collisionHandling')}</span><select value={resolveCollisions ? 'auto' : 'block'} onChange={(event) => setResolveCollisions(event.target.value === 'auto')}><option value="auto">{t('collisionAuto')}</option><option value="block">{t('collisionBlock')}</option></select></label><button type="button" className="btn primary" disabled={!canRun} onClick={() => onRun(previews)}><Play size={15} />{t('run')}</button></div>
    </div>
    ...
    <div className="structured-table-pane"><div className="structured-table-scroll table-scroll"><table className="structured-table"><thead><tr>
      ...
      {visiblePreviews.length ? visiblePreviews.map((preview) => { const status = previewStatus(preview); return <tr key={preview.fileId} className={preview.error || preview.collision ? 'row-error' : ''}><td className="structured-source-column" title={preview.inputPath}><div className="structured-source-value"><strong>{preview.before}</strong>{preview.inputPath !== preview.before && <small>{preview.inputPath}</small>}</div></td>{columns.map((column) => <td key={column.id} className="structured-cell">{isEditable(column) ? <input className="structured-cell-input" value={preview.cells[column.id] ?? ''} onChange={(event) => updateCell(preview.fileId, column.id, event.target.value)} aria-label={`${column.label} ${preview.before}`} /> : <span className="structured-generated" title={preview.cells[column.id]}>{preview.cells[column.id] || <span className="muted">{t('emptyValue')}</span>}</span>}</td>)}<td className={`structured-preview-column structured-preview-cell ${status.className}`} title={preview.outputPath}><strong>{preview.after || t('emptyValue')}</strong><small>{status.label}</small></td></tr> }) : <tr><td colSpan={columns.length + 2}><div className="filtered-empty">{t('noFilteredItems')}</div></td></tr>}
    </tbody></table></div><div className="structured-table-note"><Info size={14} /><span>{t('structuredTableHint')}</span></div></div>
