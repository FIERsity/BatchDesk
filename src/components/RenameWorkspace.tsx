...
export function RenameWorkspace({ files, busy, onBack, onRun, t }: RenameWorkspaceProps) {
  ...
  return (
    <main className="workspace task-workspace">
      <div className="task-heading"><button type="button" className="icon-btn" onClick={onBack} title={t('backInbox')}><ArrowLeft size={18} /></button><div><span>{t('inbox')} /</span><h1>{t('rename')}</h1></div><div className="heading-meta">{t('selectedCount', { count: files.length })}</div></div>
      <div className="task-layout">
        ...
        <aside className="config-pane">
          ...
          <div className="rule-list">{rules.map((rule, index) => <div key={rule.id} className={`rule-block ${rule.enabled ? '' : 'disabled'}`}>
            <div className="rule-head"><label><input type="checkbox" checked={rule.enabled} onChange={(event) => update(rule.id, { enabled: event.target.checked })} />{t(rule.type === 'case' ? 'letterCase' : rule.type)}</label><span><button type="button" className="icon-btn subtle" onClick={() => move(index, -1)} disabled={index === 0} title="Move up"><ArrowUp size={14} /></button><button type="button" className="icon-btn subtle" onClick={() => move(index, 1)} disabled={index === rules.length - 1} title="Move down"><ArrowDown size={14} /></button><button type="button" className="icon-btn subtle danger" onClick={() => setRules((current) => current.filter((item) => item.id !== rule.id))} title={t('deleteRule')}><Trash2 size={14} /></button></span></div>
