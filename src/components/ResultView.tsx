import { ArrowLeft, CheckCircle2, Download, FileJson, FolderDown, TableProperties } from 'lucide-react'
...
export function ResultView({ result, onBack, onDownload, onSaveFolder, t }: ResultViewProps) {
  ...
  return <main className="workspace result-workspace">
    <div className="result-hero"><CheckCircle2 size={34} /><div><h1>{t('complete')}</h1><p>{t('resultSummary', { success: report.totals.success, skipped: report.totals.skipped, failed: report.totals.failed })}</p></div></div>
