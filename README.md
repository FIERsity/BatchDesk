...
- 批量改名：结构化组装表左侧仅显示原始文件名，右侧显示预览文件名；中间只保留用户自行添加的序号、分隔符/固定文字和手动输入（可导入原始文件名）字段，字段可左右移动或删除，扩展名自动沿用原文件。固定文字修改会即时作用于整列，冲突自动追加 `(2)`、`(3)`，并支持撤回/恢复。另保留查找替换、前后缀、大小写、日期和高级正则规则模式
...
打开终端输出的本地地址，在浏览器中导入文件或文件夹，选择操作，预览结果后再导出。

## Usage

- 在文件收件箱中检查文件名风险并进行批量改名。
- 在 DOCX 或 XLSX 中查找和替换文字。
- 先预览变更，再导出副本、ZIP、CSV 或 JSON 报告。
...
`npm run check` runs the test, lint, and production build checks.

## Deployment

The repository publishes the production build to GitHub Pages. The deployment workflow runs `npm run build` and publishes `dist/`.
